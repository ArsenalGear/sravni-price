@if(isset($options->model) && isset($options->type))
    ...
    @if(class_exists($options->model))
        ...
        @elseif($options->type == 'belongsToMany')
            ...
            @else
               {{-- <pre>
                    @php print_r(app($options->model)->all()->toArray()) @endphp
                </pre>--}}
                <select
                ...
                >
                    ...
                    @foreach($relationshipOptions as $relationshipOption)
                        <option data-test={{$relationshipOption->name}}  value="{{ $relationshipOption->{$options->key} }}" @if(in_array($relationshipOption->{$options->key}, $selected_values)){{ 'selected="selected"' }}@endif>{{ $relationshipOption->{$options->label} }}</option>
                    @endforeach
